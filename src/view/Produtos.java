...
public class Produtos extends JDialog {
	...
	public Produtos() {
		...
		btnFoto.setBounds(881, 407, 141, 29);
		...
		btnAdicionar.setBounds(293, 368, 68, 68);
		...
		btnEditar.setBounds(371, 368, 64, 64);
		...
		btnExcluir.setBounds(445, 370, 64, 64);
		...
		btnBorracha.setBounds(519, 372, 64, 64);
	...
	private void carregarFoto() {
		...
		if (txtDesc.getText().isEmpty()) {
			...
			// lógica principal
			// CRUD Create
			String create = "INSERT INTO produtos (barcode, descricao, foto, estoque, estoquemin, valor, medida, armazenagem, idfornecedor, nome, lote, fabricante, lucro, dataent, dataval) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?)";
	...
	private void listarClientes() {
			...
			if (linha >= 0) {
				...
				try {
					...
					if (rs.next()) {
						...
						dateEnt.setDate(dataFormatadaVal);
		...
		private void editar() {
			...
			//	btnAdicionar.setEnabled(true);
				...
				// logica principal
				// CRUD - Update
				String update = "update produtos set barcode=?, descricao=?, foto=?, estoque=?, estoquemin=?, valor=?, medida=?, armazenagem=?, idfornecedor=?, nome=?, lote=?, fabricante=?, lucro=?, dataent=?, dataval=? where codigo=?";
				// trat de exceção	
				try {
					// abrir conexão
					con = dao.conectar();
